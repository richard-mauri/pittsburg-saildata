...
<div class="page-preferences" aria-label="Display preferences"><label class="wind-unit-control" for="wind-unit-select">Wind units<select id="wind-unit-select"><option value="kts" {{if eq .WindUnit "kts"}}selected{{end}}>Knots</option><option value="mph" {{if eq .WindUnit "mph"}}selected{{end}}>MPH</option></select></label></div>
<div class="grid">
...
(function(){
  ...
  var mobileOverlayPopoverQuery = window.matchMedia("(max-width: 700px)");

  function closeMobileMapOverlaysFromOutside(event) {
    if (!mapOverlaysMenu || !mapOverlaysMenu.open || !mobileOverlayPopoverQuery.matches) return;
    if (mapOverlaysMenu.contains(event.target)) return;

    // On touch screens the first outside tap should dismiss the sheet only.
    // Prevent that same tap from also selecting a map point or activating a
    // control underneath the fixed popover.
    event.preventDefault();
    event.stopPropagation();
    mapOverlaysMenu.removeAttribute("open");
  }

  document.addEventListener("pointerdown", closeMobileMapOverlaysFromOutside, true);
  document.addEventListener("keydown", function(event) {
    if (event.key === "Escape" && mapOverlaysMenu && mapOverlaysMenu.open) {
      mapOverlaysMenu.removeAttribute("open");
    }
  });
