...
func parseDistanceUnit(q url.Values) string {
	if strings.EqualFold(strings.TrimSpace(q.Get("distance_unit")), "mi") {
		return "mi"
	}
	return "nmi"
}

func distanceForDisplay(distanceNM float64, unit string) float64 {
	if unit == "mi" {
		return distanceNM * 1.15078
	}
	return distanceNM
}

func distanceUnitLabel(unit string) string {
	if unit == "mi" {
		return "mi"
	}
	return "nmi"
...
func formatDistanceNM(distanceNM float64, decimals int, unit string) string {
	return fmt.Sprintf("%.*f %s", decimals, distanceForDisplay(distanceNM, unit), distanceUnitLabel(unit))
}
...
func runServer(
	...
	mux.HandleFunc("/assets/marine_places.json", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
			return
		}
		...
		http.ServeFile(w, r, "assets/marine_places.json")
	})
	...
	// Current inland observations come from Aviation Weather Center's complete
	// METAR cache. AWC recommends its cache files for larger observation sets;
	...
	var metarCache []cachedMetarObservation

	fetchCurrentMetars := func() ([]cachedMetarObservation, error) {
		metarCacheMu.Lock()
		if !metarCacheAt.IsZero() && time.Since(metarCacheAt) < 2*time.Minute && len(metarCache) > 0 {
			cached := append([]cachedMetarObservation(nil), metarCache...)
			metarCacheMu.Unlock()
			return cached, nil
		}
		metarCacheMu.Unlock()

		req, err := http.NewRequest(http.MethodGet, "https://aviationweather.gov/data/cache/metars.cache.xml.gz", nil)
		if err != nil {
			return nil, err
		}
		req.Header.Set("Accept", "application/gzip, application/octet-stream, application/xml, text/xml")
		req.Header.Set("User-Agent", "pittsburg-saildata/"+appVersion)
		...
		var payload struct {
			Data struct {
				METARs []metarXML `xml:"METAR"`
			} `xml:"data"`
		}
		decoder := xml.NewDecoder(io.LimitReader(gz, 32<<20))
		if err := decoder.Decode(&payload); err != nil {
			return nil, fmt.Errorf("XML: %w", err)
		}

		result := make([]cachedMetarObservation, 0, len(payload.Data.METARs))
		for _, m := range payload.Data.METARs {
			station := strings.ToUpper(strings.TrimSpace(m.Station))
			lat, latErr := strconv.ParseFloat(strings.TrimSpace(m.Lat), 64)
			lon, lonErr := strconv.ParseFloat(strings.TrimSpace(m.Lon), 64)
			wspd, speedErr := strconv.ParseFloat(strings.TrimSpace(m.Wspd), 64)
			if station == "" || latErr != nil || lonErr != nil || speedErr != nil ||
				math.IsNaN(lat) || math.IsNaN(lon) || math.IsNaN(wspd) ||
				math.IsInf(lat, 0) || math.IsInf(lon, 0) || math.IsInf(wspd, 0) || wspd < 0 {
				continue
			}
			obsTime, timeErr := time.Parse(time.RFC3339, strings.TrimSpace(m.ObsTime))
			if timeErr != nil {
				obsTime = time.Time{}
			}
			item := cachedMetarObservation{Station: station, Time: obsTime, Lat: lat, Lon: lon, Wspd: wspd}
			if wdir, err := strconv.ParseFloat(strings.TrimSpace(m.Wdir), 64); err == nil &&
				!math.IsNaN(wdir) && !math.IsInf(wdir, 0) && wdir >= 0 && wdir <= 360 {
				item.Wdir = math.Mod(wdir+360, 360)
				item.HasWdir = true
			}
			if wgst, err := strconv.ParseFloat(strings.TrimSpace(m.Wgst), 64); err == nil &&
		...
		if len(result) == 0 {
			return nil, fmt.Errorf("cache contained no usable METAR observations")
		}

		metarCacheMu.Lock()
		metarCacheAt = time.Now()
		metarCache = append([]cachedMetarObservation(nil), result...)
		metarCacheMu.Unlock()
		return result, nil
	}

	mux.HandleFunc("/wind-barbs", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
			return
		}
		...
		if minLat < -90 || maxLat > 90 || minLat >= maxLat || minLon < -180 || maxLon > 180 || minLon >= maxLon {
			http.Error(w, "invalid wind-barb bounds", http.StatusBadRequest)
			return
		}

		includeMarine := q.Get("include_marine") != "0"
		includeInland := q.Get("include_inland") == "1"

		type windBarbObservation struct {
			Station        string  `json:"station"`
			Name           string  `json:"name"`
		...
		items := make([]windBarbObservation, 0)
		...
		if includeMarine {
			...
			if stationErr != nil {
				...
				type stationResult struct {
					item windBarbObservation
					ok   bool
				}
				type windBarbNDBCCandidate struct {
					ID   string
					Name string
					Lat  float64
					Lon  float64
				}
				candidates := make([]windBarbNDBCCandidate, 0)
				for _, station := range stations {
					if station.Lat >= minLat && station.Lat <= maxLat && station.Lon >= minLon && station.Lon <= maxLon {
						candidates = append(candidates, windBarbNDBCCandidate{ID: station.ID, Name: station.Name, Lat: station.Lat, Lon: station.Lon})
					}
				}
				...
				for _, station := range candidates {
					...
					go func() {
						defer wg.Done()
						sem <- struct{}{}
						stationID := strings.ToUpper(strings.TrimSpace(station.ID))
						wind, age := latestNearbyStationWind(stationID, windUnit)
						<-sem
						if strings.TrimSpace(wind) == "" {
							return
						}
						results <- stationResult{item: windBarbObservation{
							Station: stationID, Name: station.Name, Wind: wind, ObservationAge: age,
							Lat: station.Lat, Lon: station.Lon, Class: "marine", Source: "NOAA/NDBC",
						}, ok: true}
					}()
				}
				wg.Wait()
		...
		if includeInland {
			...
			if metarErr != nil {
				...
				for _, m := range metars {
					if m.Lat < minLat || m.Lat > maxLat || m.Lon < minLon || m.Lon > maxLon {
						continue
					}
					if m.Wspd >= 3 && !m.HasWdir {
						continue
					}
					directionDeg := 0.0
					if m.HasWdir {
					...
					idx := int(math.Floor((directionDeg+11.25)/22.5)) % len(compass)
					windText := fmt.Sprintf("%s %.0f %s", compass[idx], windSpeedForDisplay(m.Wspd, windUnit), windUnitLabel(windUnit))
					if m.HasWgst && m.Wgst > m.Wspd {
						windText += fmt.Sprintf(" G%.0f", windSpeedForDisplay(m.Wgst, windUnit))
					}
					ageText := ""
					if !m.Time.IsZero() {
						ageMinutes := int(math.Round(time.Since(m.Time.UTC()).Minutes()))
						if ageMinutes < 0 {
							ageMinutes = 0
						}
						if ageMinutes < 60 {
							ageText = fmt.Sprintf("%d min", ageMinutes)
						} else {
							ageText = fmt.Sprintf("%dh %02dm", ageMinutes/60, ageMinutes%60)
						}
					}
					items = append(items, windBarbObservation{
						Station: m.Station, Name: m.Station, Wind: windText, ObservationAge: ageText,
						Lat: m.Lat, Lon: m.Lon, Class: "inland", Source: "Aviation Weather Center METAR",
					})
				}
			}
		}
	...
	mux.HandleFunc("/wind-stations", func(w http.ResponseWriter, r *http.Request) {
		...
		windUnit := parseWindUnit(q)
		distanceUnit := parseDistanceUnit(q)
		...
		for _, c := range candidates {
			...
			item := stationMapCandidate{
				...
				Name:     c.Station.Name,
				Distance: formatDistanceNM(c.SelectedDistance, 1, distanceUnit),
				Lat:      c.Station.Lat,
				Lon:      c.Station.Lon,
			...
			default:
				...
				item.CurrentDistance = formatDistanceNM(currentPreview.DistanceNM, 1, distanceUnit)
...
func makeHTMLReportData(report *SailingReport, loc *time.Location) htmlReportData {
	...
	d.DistanceUnit = parseDistanceUnit(report.RequestQuery)
	...
	if _, _, hasUserLocation, _ := parseOptionalLatLon(report.RequestQuery); report.WindSelection != nil && hasUserLocation {
		for i, candidate := range report.WindSelection.Candidates {
			...
			item := htmlWindCandidate{
				...
				Distance:   formatDistanceNM(candidate.DistanceNM, 1, d.DistanceUnit),
				Lat:        candidate.Lat,
				Lon:        candidate.Lon,
				Met:        candidate.Met,
				Status:     strings.ToUpper(candidate.WindStatus),
			...
			default:
				...
				item.CurrentStation = currentPreview.ID
				item.CurrentName = currentPreview.Name
				item.CurrentDistance = formatDistanceNM(currentPreview.DistanceNM, 1, d.DistanceUnit)
				item.CurrentLat = currentPreview.Lat
				item.CurrentLon = currentPreview.Lon
			}
...
.map-menu-close{display:inline-flex;align-items:center;justify-content:center;position:sticky;top:0;align-self:flex-end;z-index:2;width:36px;height:36px;margin:-6px -6px -2px 0;border:0;border-radius:999px;background:#edf4f6;color:var(--navy);font:inherit;font-size:1.35rem;font-weight:900;line-height:1;cursor:pointer;flex:0 0 auto}
...
.place-search{margin:12px 0 10px}.place-search-form{display:flex;gap:8px;align-items:stretch;max-width:760px}.place-search-input{flex:1;min-width:0;padding:10px 12px;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--ink);font:inherit}.place-search-input:focus{outline:2px solid rgba(18,107,145,.25);border-color:#126b91}.place-search-submit{padding:10px 16px;border:1px solid #126b91;border-radius:10px;background:#126b91;color:#fff;font:inherit;font-weight:850;cursor:pointer}.place-search-submit:disabled{opacity:.6;cursor:wait}.place-search-status{min-height:1.3em;margin-top:6px;color:var(--muted);font-size:.8rem}.place-search-results{display:grid;gap:5px;max-width:760px;max-height:280px;margin-top:6px;overflow-y:auto;-webkit-overflow-scrolling:touch;padding-right:3px}.place-search-results:empty{display:none}.place-search-result{width:100%;padding:9px 11px;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--ink);font:inherit;text-align:left;cursor:pointer;line-height:1.3}.place-search-result:hover,.place-search-result:focus{border-color:#126b91;background:#f2f9fb;outline:none}.place-search-result-name{display:block;font-weight:800;color:var(--navy)}.place-search-result-type{display:block;margin-top:2px;color:var(--muted);font-size:.75rem;text-transform:capitalize}.place-search-note{max-width:760px;margin:5px 0 0;color:var(--muted);font-size:.72rem}.place-search-note a{color:var(--blue)}@media(max-width:600px){.place-search-form{display:grid;grid-template-columns:1fr auto}.place-search-submit{padding-left:13px;padding-right:13px}.place-search-results{max-height:220px}.place-search-result{padding:10px 11px}}
...
<div class="wind-readings-scroll-shell"><div id="wind-readings-scroll" class="wind-readings-wrap"><table class="wind-readings-table"><thead><tr><th>Time</th><th>Dir</th><th>Wind</th><th>Gust</th><th>Age</th></tr></thead><tbody id="wind-readings-body">{{range .WindReadings}}<tr><td>{{.Time}}</td><td>{{.Direction}}</td><td>{{.Wind}}</td><td>{{.Gust}}</td><td>{{.Age}}</td></tr>{{end}}</tbody></table></div><div id="wind-readings-scroll-cue" class="wind-readings-scroll-cue" aria-hidden="true">Scroll for more ↓</div></div>
</div>{{end}}
</section>
...
(function(){
  ...
  var marinePlacesData = null;
  var marinePlaceLayers = Object.create(null);
  var marinePlaceToggles = Array.from(document.querySelectorAll(".map-marine-place-toggle"));
  var marinePlaceColors = {
    marinas: "#126b91",
    boatyards: "#7a4a21",
    fuel_docks: "#347a4b",
    launch_ramps: "#376f9c",
    marine_supply: "#805f2f",
    yacht_clubs: "#6c4a91",
    waterfront_restaurants: "#a45135"
  };

  function marinePlacePopup(place) {
    var wrap = document.createElement("div");
    var title = document.createElement("strong");
    title.textContent = String(place.name || "Marine place");
    wrap.appendChild(title);
    if (place.city) {
      var city = document.createElement("div");
      city.textContent = String(place.city);
      city.style.marginTop = "3px";
      wrap.appendChild(city);
    }
    if (place.address) {
      var address = document.createElement("div");
      address.textContent = String(place.address);
      address.style.marginTop = "4px";
      address.style.fontSize = ".82rem";
      wrap.appendChild(address);
    }
    if (place.note) {
      var note = document.createElement("div");
      note.textContent = String(place.note);
    ...
    if (place.website) {
      try {
        var websiteURL = new URL(String(place.website), window.location.href);
        if (websiteURL.protocol === "http:" || websiteURL.protocol === "https:") {
          var websiteRow = document.createElement("div");
          websiteRow.style.marginTop = "6px";
          websiteRow.style.fontSize = ".84rem";
  ...
  function ensureMarinePlaceLayer(category) {
    if (marinePlaceLayers[category]) return marinePlaceLayers[category];
    var layer = L.layerGroup();
    marinePlaceLayers[category] = layer;
    if (!marinePlacesData || !Array.isArray(marinePlacesData.places)) return layer;
    marinePlacesData.places.forEach(function(place) {
      ...
      var marker = L.circleMarker([lat, lon], {
        ...
        color: "#ffffff",
        weight: 2.5,
        fillColor: marinePlaceColors[category] || "#126b91",
        fillOpacity: .98
      });
      marker.bindPopup(marinePlacePopup(place));
      marker.addTo(layer);
    });
    return layer;
  }

  function syncMarinePlaceOverlays() {
    marinePlaceToggles.forEach(function(toggle) {
      var category = toggle.getAttribute("data-marine-category") || "";
      var layer = ensureMarinePlaceLayer(category);
      if (toggle.checked) {
  ...
  fetch("/assets/marine_places.json?build=v248", {headers:{"Accept":"application/json"}, cache:"no-store"})
    ...
    .then(function(payload) {
      marinePlacesData = payload || {places:[]};
      var counts = Object.create(null);
      if (Array.isArray(marinePlacesData.places)) {
        marinePlacesData.places.forEach(function(place) {
      ...
      marinePlaceToggles.forEach(function(toggle) {
        var category = toggle.getAttribute("data-marine-category") || "";
        var countEl = document.getElementById("map-marine-count-" + category);
        if (countEl) countEl.textContent = "(" + String(counts[category] || 0) + ")";
      });
      syncMarinePlaceOverlays();
    })
    .catch(function(err) {
      console.error("Marine places could not be loaded:", err);
      marinePlaceToggles.forEach(function(toggle) { toggle.disabled = true; });
    });

  var mapScaleStatus = document.getElementById("map-scale-status");
  ...
  function currentMapDistanceUnit() {
    var unitSelect = document.getElementById("distance-unit-select");
    if (unitSelect && unitSelect.value === "mi") return "mi";
    if (unitSelect && unitSelect.value === "nmi") return "nmi";
    var urlUnit = pageURL.searchParams.get("distance_unit");
    if (urlUnit === "mi") return "mi";
    if (urlUnit === "nmi") return "nmi";
    return {{printf "%q" .DistanceUnit}} === "mi" ? "mi" : "nmi";
  }

  function updateMapScaleStatus() {
    if (!mapScaleStatus) return;
    ...
    var distanceUnit = currentMapDistanceUnit();
    var distance = distanceUnit === "mi" ? meters / 1609.344 : meters / 1852;
    var label = distanceUnit === "mi" ? "mi" : "nmi";
    mapScaleStatus.textContent =
      "Scale: " + formatMapScaleDistance(distance) + " " + label +
      " · Zoom " + map.getZoom();
  }

  var mapDistanceUnitSelect = document.getElementById("distance-unit-select");
  if (mapDistanceUnitSelect) {
    mapDistanceUnitSelect.addEventListener("change", updateMapScaleStatus);
  }
  ...
  map.getPane("windBarbPane").style.zIndex = 625;
  ...
  var locationHelp = document.querySelector("details.location-help");
  var locationHelpClose = locationHelp ? locationHelp.querySelector(".location-help-close") : null;

  function closeLocationHelp() {
    if (locationHelp && locationHelp.open) locationHelp.removeAttribute("open");
  }

  if (locationHelpClose) {
    locationHelpClose.addEventListener("click", function(event) {
  ...
  // Treat an outside tap as dismissal only. This makes the help sheet predictable
  // on iOS and prevents the same tap from also selecting a map point or activating
  // a control underneath it. Taps inside the help, including its summary, retain
  // normal native <details> behavior.
  document.addEventListener("pointerdown", function(event) {
    if (!locationHelp || !locationHelp.open || locationHelp.contains(event.target)) return;
    event.preventDefault();
    event.stopPropagation();
    closeLocationHelp();
  }, true);

  var tideContextModal = document.getElementById("tide-context-modal");
  var tideContextOpen = document.getElementById("tide-context-open");
  var tideContextClose = document.getElementById("tide-context-close");

  function openTideContext() {
    if (!tideContextModal) return;
    tideContextModal.hidden = false;
    document.body.classList.add("tide-context-modal-open");
    window.setTimeout(function() {
      if (tideContextClose) tideContextClose.focus();
    }, 0);
  }

  function closeTideContext() {
    if (!tideContextModal || tideContextModal.hidden) return;
    tideContextModal.hidden = true;
    document.body.classList.remove("tide-context-modal-open");
    if (tideContextOpen) tideContextOpen.focus();
  }

  if (tideContextOpen) {
    tideContextOpen.addEventListener("click", function(event) {
      event.preventDefault();
      openTideContext();
    });
  }
  if (tideContextClose) {
    tideContextClose.addEventListener("click", function(event) {
      event.preventDefault();
      closeTideContext();
    });
  }
  if (tideContextModal) {
    tideContextModal.addEventListener("pointerdown", function(event) {
      if (event.target !== tideContextModal) return;
      event.preventDefault();
      event.stopPropagation();
      closeTideContext();
    });
  ...
  function wireInfoPopup(openID, modalID, closeID) {
    ...
    function closePopup() {
      ...
      modal.hidden = true;
      if (!document.querySelector(".tide-context-modal:not([hidden]), .info-popup-modal:not([hidden])")) {
        document.body.classList.remove("tide-context-modal-open");
      }
      openButton.focus();
    }
  ...
  var closeCurrentInfo = wireInfoPopup("current-info-open", "current-info-modal", "current-info-close");
  var closePlanningInfo = wireInfoPopup("planning-info-open", "planning-info-modal", "planning-info-close");
  var closeMapLegendInfo = wireInfoPopup("map-legend-open", "map-legend-modal", "map-legend-close");
  var closeMapSourcesInfo = wireInfoPopup("map-sources-open", "map-sources-modal", "map-sources-close");

  var mapMenuIDs = ["map-types-menu", "map-overlays-menu", "map-center-menu"];
  var mobileMapMenuQuery = window.matchMedia("(max-width: 700px)");

  function getOpenMapMenu() {
    for (var i = 0; i < mapMenuIDs.length; i++) {
      var menu = document.getElementById(mapMenuIDs[i]);
      if (menu && menu.open) return menu;
    }
    return null;
  }

  function closeMapMenu(menu) {
    if (menu && menu.open) menu.removeAttribute("open");
  }

  // Keep native <details>/<summary> opening behavior untouched. Close buttons
  // are now available on desktop and mobile, and only close their own menu.
  document.querySelectorAll(".map-overlays-menu .map-menu-close").forEach(function(button) {
    button.addEventListener("click", function(event) {
      event.preventDefault();
      event.stopPropagation();
      closeMapMenu(button.closest("details.map-overlays-menu"));
    });
  });

  var clearMapOverlaysButton = document.getElementById("map-overlays-clear");
  if (clearMapOverlaysButton) {
    clearMapOverlaysButton.addEventListener("click", function(event) {
      event.preventDefault();
      event.stopPropagation();
      var overlayMenu = document.getElementById("map-overlays-menu");
      if (!overlayMenu) return;
      overlayMenu.querySelectorAll('input[type="checkbox"]').forEach(function(toggle) {
  ...
  function closeMapMenuFromOutside(event) {
    var openMenu = getOpenMapMenu();
    if (!openMenu || openMenu.contains(event.target)) return;

    // On narrow/touch layouts, the first outside tap is dismissal-only so it
    // cannot also select a map point or activate a control underneath. Desktop
    // clicks close the menu without suppressing the clicked control.
    if (mobileMapMenuQuery.matches) {
      event.preventDefault();
      event.stopPropagation();
    }
  ...
  document.addEventListener("pointerdown", closeMapMenuFromOutside, true);
  ...
  var windBarbLayer = L.layerGroup();
  var windBarbObservations = [];
  var windBarbObservationCache = Object.create(null);
  ...
  function setWindBarbStatus(message, isError) {
    var status = document.getElementById("map-windbarb-status");
    if (!status) return;
    message = String(message || "").trim();
    status.hidden = !message;
    status.textContent = message;
  ...
  function windDirectionDegrees(value) {
    ...
    return Object.prototype.hasOwnProperty.call(points, key) ? points[key] : null;
  }

  function parseCandidateWind(windText) {
    var text = String(windText || "").trim();
    if (!text) return null;
    var parts = text.split(/\s+/);
    if (parts.length < 2) return null;
    var degrees = windDirectionDegrees(parts[0]);
    var speed = Number(parts[1]);
    if (degrees === null || !Number.isFinite(speed)) return null;
    if (/\bmph\b/i.test(text)) speed = speed / 1.150779448;
    return {direction:parts[0].toUpperCase(), degrees:degrees, speedKT:Math.max(0, speed)};
  }

  function windBarbSVG(speedKT, degrees) {
    var rounded = Math.max(0, Math.round(Number(speedKT) / 5) * 5);
    if (rounded < 3) {
      return '<svg width="34" height="34" viewBox="-17 -17 34 34" aria-hidden="true"><circle class="barb-stroke" cx="0" cy="0" r="5"></circle></svg>';
    }
    var remaining = rounded;
    var y = -13;
    var marks = '';
    while (remaining >= 50) {
      marks += '<path class="barb-flag" d="M0 '+y+' L9 '+(y+4)+' L0 '+(y+7)+' Z"></path>';
      y += 8;
      remaining -= 50;
    }
    while (remaining >= 10) {
      marks += '<path class="barb-stroke" d="M0 '+y+' L9 '+(y+4)+'"></path>';
      y += 4;
      remaining -= 10;
    }
    if (remaining >= 5) marks += '<path class="barb-stroke" d="M0 '+y+' L5 '+(y+2)+'"></path>';
    return '<svg width="42" height="42" viewBox="-21 -21 42 42" aria-hidden="true"><g transform="rotate('+Number(degrees).toFixed(1)+')"><path class="barb-stroke" d="M0 12 L0 -14"></path>'+marks+'</g><circle cx="0" cy="0" r="2.2" fill="#102f44"></circle></svg>';
  }

  function windBarbSpacingPixels() {
    var zoom = map.getZoom();
    if (zoom <= 5) return 38;
    if (zoom <= 6) return 34;
    if (zoom <= 7) return 30;
  ...
  function windBarbAgeMinutes(value) {
    var ageText = String(value || "");
    var ageMinutes = 0;
    var hourMatch = ageText.match(/(\d+)h/i);
    var minuteMatch = ageText.match(/(\d+)\s*min/i);
    if (hourMatch) ageMinutes += Number(hourMatch[1]) * 60;
    if (minuteMatch) ageMinutes += Number(minuteMatch[1]);
    if (!hourMatch && !minuteMatch) {
      var simpleMatch = ageText.match(/(\d+)/);
      if (simpleMatch) ageMinutes = Number(simpleMatch[1]);
    }
    return Number.isFinite(ageMinutes) ? ageMinutes : 999999;
  }

  function windBarbCacheKey(c) {
    return String(c.class || "marine").toLowerCase() + ":" +
      normalizeWindStationID(c.station || "");
  }

  function refreshWindBarbObservationArray() {
    var now = Date.now();
    var next = [];
    Object.keys(windBarbObservationCache).forEach(function(key) {
      var entry = windBarbObservationCache[key];
      if (!entry || !Number.isFinite(entry.cachedAt) || now - entry.cachedAt > windBarbCacheMaxAgeMS) {
        delete windBarbObservationCache[key];
        return;
      }
      next.push(entry.observation);
    });
    windBarbObservations = next;
  }

  function mergeWindBarbObservations(observations) {
    var now = Date.now();
    observations.forEach(function(c) {
      var observation = {
        station: normalizeWindStationID(c.station),
        name: String(c.name || c.station || ""),
        wind: String(c.wind || ""),
        observationAge: String(c.observation_age || ""),
        class: String(c.class || "marine"),
        source: String(c.source || ""),
        lat: Number(c.lat),
        lon: Number(c.lon)
      };
      var key = windBarbCacheKey(observation);
      if (!observation.station || !key) return;
      windBarbObservationCache[key] = {observation: observation, cachedAt: now};
    });
    refreshWindBarbObservationArray();
  }

  function renderWindBarbs() {
    windBarbLayer.clearLayers();
    if (!mapState.windBarbsVisible && !mapState.inlandWindBarbsVisible) return;
    refreshWindBarbObservationArray();
    var usable = 0;
    var marineUsable = 0;
    var inlandUsable = 0;
    ...
    var spacing = windBarbSpacingPixels();
    var zoom = map.getZoom();

    // Use Leaflet world-pixel coordinates rather than viewport-relative pixels.
    // At a fixed zoom, the same station always belongs to the same thinning cell
    // no matter how the user pans the map. This prevents small pans from swapping
    // one nearby barb for another.
    var ordered = windBarbObservations.slice().sort(function(a, b) {
      var ac = String(a.class || "marine").toLowerCase() === "inland" ? 1 : 0;
      var bc = String(b.class || "marine").toLowerCase() === "inland" ? 1 : 0;
      if (ac !== bc) return ac - bc;
      var ageDiff = windBarbAgeMinutes(a.observationAge) - windBarbAgeMinutes(b.observationAge);
      if (ageDiff !== 0) return ageDiff;
      return normalizeWindStationID(a.station).localeCompare(normalizeWindStationID(b.station));
    });

    ordered.forEach(function(c) {
      var stationClass = String(c.class || "marine").toLowerCase();
      if (stationClass === "inland" && !mapState.inlandWindBarbsVisible) return;
      if (stationClass !== "inland" && !mapState.windBarbsVisible) return;
      var parsed = parseCandidateWind(c.wind);
      var lat = Number(c.lat), lon = Number(c.lon);
      if (!parsed || !Number.isFinite(lat) || !Number.isFinite(lon)) return;

      var p = map.project(L.latLng(lat, lon), zoom);
      var cell = Math.floor(p.x / spacing) + ":" + Math.floor(p.y / spacing);
      if (occupied[cell]) {
        thinned++;
        return;
      }
      ...
      var ageMinutes = windBarbAgeMinutes(c.observationAge);
      var isStale = ageMinutes > 90;
      if (isStale) stale++;
      var icon = L.divIcon({
        className:"wind-barb-icon" + (isStale ? " stale" : ""),
        html:windBarbSVG(parsed.speedKT, parsed.degrees),
        iconSize:[42,42], iconAnchor:[21,21]
      });
      var marker = L.marker([lat,lon], {icon:icon,pane:"windBarbPane",keyboard:false,zIndexOffset:80});
      var sourceLabel = c.source ? " · " + c.source : "";
      marker.bindTooltip(String(c.station || "") + " · " + String(c.wind || "") + (c.observationAge ? " · " + c.observationAge + " old" : "") + sourceLabel);
      marker.addTo(windBarbLayer);
      usable++;
      if (stationClass === "inland") inlandUsable++; else marineUsable++;
    });
    if (!map.hasLayer(windBarbLayer)) windBarbLayer.addTo(map);
    if (!usable) {
      var emptyMessage = "Wind barbs · no usable observations returned for the enabled layers in the current map view.";
      if (windBarbDataErrors.length) emptyMessage += " " + windBarbDataErrors.join(" | ");
      setWindBarbStatus(emptyMessage, windBarbDataErrors.length > 0);
    } else {
      var parts = ["Observed wind barbs", usable + " shown"];
      if (mapState.windBarbsVisible) parts.push(marineUsable + " NOAA/NDBC marine/Bay");
      if (mapState.inlandWindBarbsVisible) parts.push(inlandUsable + " METAR land/inland");
      if (thinned) parts.push(thinned + " hidden at this zoom to reduce overlap");
      if (stale) parts.push(stale + " older than 90 min faded");
      if (windBarbDataErrors.length) parts.push(windBarbDataErrors.length + " source warning" + (windBarbDataErrors.length === 1 ? "" : "s"));
      setWindBarbStatus(parts.join(" · "), false);
    }
  }

  function windBarbBoundsContain(outer, inner) {
  ...
  function bufferedWindBarbBounds(bounds) {
    var latPad = Math.max(0.08, (bounds.getNorth() - bounds.getSouth()) * 0.70);
    var lonPad = Math.max(0.08, (bounds.getEast() - bounds.getWest()) * 0.70);
    return L.latLngBounds(
      [Math.max(-90, bounds.getSouth() - latPad), Math.max(-180, bounds.getWest() - lonPad)],
      [Math.min(90, bounds.getNorth() + latPad), Math.min(180, bounds.getEast() + lonPad)]
    );
  }

  function loadWindBarbsForViewport(force) {
    if (!mapState.windBarbsVisible && !mapState.inlandWindBarbsVisible) return;

    var visibleBounds = map.getBounds();
    if (!visibleBounds || !visibleBounds.isValid()) {
      setWindBarbStatus("Wind barbs · map bounds unavailable.", true);
      return;
    }
    if (!force && windBarbBoundsContain(windBarbLoadedBounds, visibleBounds)) {
      renderWindBarbs();
      return;
    }
    ...
    var endpoint =
      ...
      "&max_lat=" + encodeURIComponent(requestBounds.getNorth().toFixed(5)) +
      "&min_lon=" + encodeURIComponent(requestBounds.getWest().toFixed(5)) +
      "&max_lon=" + encodeURIComponent(requestBounds.getEast().toFixed(5)) +
      "&wind_unit={{.WindUnit}}" +
      "&include_marine=" + (mapState.windBarbsVisible ? "1" : "0") +
      "&include_inland=" + (mapState.inlandWindBarbsVisible ? "1" : "0");
    ...
    fetch(endpoint, {headers:{"Accept":"application/json"}})
      ...
      .then(function(payload) {
        ...
        windBarbLoadedBounds = requestBounds;
        renderWindBarbs();
      })
      .catch(function(err) {
        if ((!mapState.windBarbsVisible && !mapState.inlandWindBarbsVisible) || requestSerial !== windBarbRequestSerial) return;
  ...
  function scheduleWindBarbRefresh() {
    if (!mapState.windBarbsVisible && !mapState.inlandWindBarbsVisible) return;
    if (windBarbRefreshTimer) window.clearTimeout(windBarbRefreshTimer);
    windBarbRefreshTimer = window.setTimeout(function() {
      windBarbRefreshTimer = null;
      loadWindBarbsForViewport();
    }, 250);
  }

  function setWindBarbsVisible(visible) {
    ...
    if (!mapState.windBarbsVisible && !mapState.inlandWindBarbsVisible) {
      ...
      windBarbObservations = [];
      windBarbObservationCache = Object.create(null);
      windBarbDataErrors = [];
      windBarbLoadedBounds = null;
      windBarbLayer.clearLayers();
      if (map.hasLayer(windBarbLayer)) map.removeLayer(windBarbLayer);
      setWindBarbStatus("", false);
      return;
    }
    if (mapState.windBarbsVisible) loadWindBarbsForViewport(true); else renderWindBarbs();
  }

  function setInlandWindBarbsVisible(visible) {
    ...
    if (mapState.inlandWindBarbsVisible) loadWindBarbsForViewport(true); else renderWindBarbs();
  }
  ...
  function renderWindMarkers() {
    ...
    // Wind barbs are loaded independently for the current viewport.
  }
  ...
  var windBarbCheckbox = document.getElementById("map-show-wind-barbs");
  if (windBarbCheckbox) {
    windBarbCheckbox.checked = !!mapState.windBarbsVisible;
    windBarbCheckbox.addEventListener("change", function() {
      setWindBarbsVisible(!!windBarbCheckbox.checked);
    });
  }

  var inlandWindBarbCheckbox = document.getElementById("map-show-inland-wind-barbs");
  if (inlandWindBarbCheckbox) {
    inlandWindBarbCheckbox.checked = !!mapState.inlandWindBarbsVisible;
    inlandWindBarbCheckbox.addEventListener("change", function() {
      setInlandWindBarbsVisible(!!inlandWindBarbCheckbox.checked);
    });
  }
  ...
  function renderSearchControls() {
    if (findPoint) {
      ...
      if (!mapState.selectedLocation) {
        findPoint.textContent = "Find nearby stations";
  ...
  if (reset) {
    reset.addEventListener("click", function() {
      ...
      if (selectedWindMarker) {
        map.removeLayer(selectedWindMarker);
        selectedWindMarker = null;
      }
      if (currentStationMarker) {
        map.removeLayer(currentStationMarker);
        currentStationMarker = null;
      }
      selectedCurrentLatLng = null;
      ...
      if (window.history && window.history.replaceState) {
        ...
        resetURL.searchParams.delete("station");
        resetURL.searchParams.delete("current_station");
        resetURL.searchParams.delete("bin");
...
(function(){
  ...
  unitSelect.addEventListener("change", function() {
    var target = new URL(window.location.href);
    target.searchParams.set("distance_unit", unitSelect.value === "mi" ? "mi" : "nmi");
    window.location.href = target.toString();
  });
})();
...
(function(){
  ...
  var readingsScroll = document.getElementById("wind-readings-scroll");
  var readingsScrollCue = document.getElementById("wind-readings-scroll-cue");
  ...
  function updateReadingsScrollCue() {
    if (!readingsScroll || !readingsScrollCue) return;
    var shell = readingsScroll.closest(".wind-readings-scroll-shell");
    if (!shell) return;
    var overflow = readingsScroll.scrollHeight > readingsScroll.clientHeight + 2;
    var atEnd = readingsScroll.scrollTop + readingsScroll.clientHeight >= readingsScroll.scrollHeight - 3;
    shell.classList.toggle("has-more", overflow && !atEnd);
  }
  ...
  function renderReadings(readings) {
    ...
    if (readingsScroll) readingsScroll.scrollTop = 0;
    window.requestAnimationFrame(updateReadingsScrollCue);
  }
  ...
  function renderWindChart(readings) {
    ...
    // Keep SVG user units close to actual CSS pixels on narrow screens. The old
    // fixed 760-unit viewBox was scaled down heavily on iPhone, shrinking its
    // axis labels along with the plot.
    var narrowChart = window.matchMedia("(max-width: 640px)").matches;
    var measuredWidth = Math.round(chart.getBoundingClientRect().width || chart.clientWidth || 0);
    var width = narrowChart ? Math.max(320, Math.min(520, measuredWidth - 16)) : 760;
    var height = narrowChart ? 280 : 300;
    var left = narrowChart ? 46 : 42;
    var right = narrowChart ? 12 : 14;
    var top = 12;
    var bottom = narrowChart ? 46 : 42;
    ...
    function chooseLabelIndexes(count) {
      if (count <= 1) return [0];
      // Four or fewer x-axis labels are much easier to read on an iPhone.
      var desired = narrowChart
        ? Math.min(4, Math.max(3, Math.round(count / 10)))
        : Math.min(6, Math.max(4, Math.round(count / 8)));
    ...
    chart.innerHTML =
      ...
      xLabels +
      '<g id="wind-chart-inspector" style="display:none">' +
      '<line class="wind-chart-cursor" x1="' + left + '" y1="' + top + '" x2="' + left + '" y2="' + (height-bottom) + '"></line>' +
      '<circle class="wind-chart-cursor-dot wind" cx="' + left + '" cy="' + top + '" r="4" style="display:none"></circle>' +
      '<circle class="wind-chart-cursor-dot gust" cx="' + left + '" cy="' + top + '" r="4" style="display:none"></circle>' +
      '</g>' +
  ...
  if (readingsScroll) {
    readingsScroll.addEventListener("scroll", updateReadingsScrollCue, {passive:true});
    window.addEventListener("resize", updateReadingsScrollCue);
    window.requestAnimationFrame(updateReadingsScrollCue);
  }
